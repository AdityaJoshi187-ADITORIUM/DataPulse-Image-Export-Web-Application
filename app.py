...
from helper import data, seconddata, match_elements, describe, outliers, drop_items, download_data, filter_data, num_filter_data, rename_columns, clear_image_cache, handling_missing_values, data_wrangling, plot_3d_scatter, plot_3d_bar, plot_3d_line, plot_3d_area, plot_3d_pie, plot_3d_bubble, plot_3d_treemap, plot_3d_heatmap
import numpy as np
import pandas as pd
from helper import *
...
st.set_page_config(
    page_title="DataPulse Image Export Web Application",
    page_icon="🧊",
    layout="wide",
    initial_sidebar_state="expanded",
    menu_items={
        'Get help': 'https://github.com/AdityaJoshi187-ADITORIUM/DataPulse-Image-Export-Web-Application',
        'Report a bug': 'https://github.com/AdityaJoshi187-ADITORIUM/DataPulse-Image-Export-Web-Application/issues/new',
        'About': "# 🤖 Welcome to the Data Clinic!! 🤖\n\n"
                 "This application allows you to analyze and export images "
                 "from various data sources. Explore insights and visualize data "
                 "like never before."
...
st.sidebar.title("DataPulse-Image-Export-Web-Application")

file_format_type = ["csv", "txt", "xls", "xlsx", "ods", "odt"]
functions = ["Overview", "Outliers", "Drop Columns", "Drop Categorical Rows", "Drop Numeric Rows", "Rename Columns", "Display Plot", "Handling Missing Data", "Data Wrangling", "3D Visualizations"]
...
# ==================================================================================================
    if "Outliers" in multi_function_selector:
        ...
        outliers_figs = outliers(data, outliers_selection)
        
        for fig in outliers_figs:
            st.pyplot(fig)
# ===================================================================================================
...
# ====================================================================================================================  
    ...
    if "3D Visualizations" in multi_function_selector:
        st.subheader("3D Visualizations")

        plot_type = st.selectbox("Select 3D plot type", ["Scatter", "Bar", "Line", "Area", "Pie", "Bubble", "Treemap", "Heatmap", "Surface", "Contour"])
        x_axis = st.selectbox("Select X-axis", data.columns)
        y_axis = st.selectbox("Select Y-axis", data.columns)
        z_axis = st.selectbox("Select Z-axis", data.columns)

        if plot_type == "Scatter":
            fig = plot_3d_scatter(data, x_axis, y_axis, z_axis)
        elif plot_type == "Bar":
            fig = plot_3d_bar(data, x_axis, y_axis, z_axis)
        elif plot_type == "Line":
            fig = plot_3d_line(data, x_axis, y_axis, z_axis)
        elif plot_type == "Area":
            fig = plot_3d_area(data, x_axis, y_axis, z_axis)
        elif plot_type == "Pie":
            fig = plot_3d_pie(data, x_axis, y_axis, z_axis)
        elif plot_type == "Bubble":
            fig = plot_3d_bubble(data, x_axis, y_axis, z_axis)
        elif plot_type == "Treemap":
            fig = plot_3d_treemap(data, x_axis, y_axis, z_axis)
        elif plot_type == "Heatmap":
            fig = plot_3d_heatmap(data, x_axis, y_axis, z_axis)
        elif plot_type == "Surface":
            fig = plot_3d_surface(data, x_axis, y_axis, z_axis)
        elif plot_type == "Contour":
            fig = plot_3d_contour(data, x_axis, y_axis, z_axis)
...
if st.sidebar.button("Clear Cache"):
    clear_image_cache()
st.sidebar.title("Image Dataset Downloader")
st.sidebar.subheader("Download Images for Dataset")
image_query = st.sidebar.text_input("Enter Image Query:")
image_limit = st.sidebar.number_input("Enter Number of Images to Download:", min_value=1, max_value=100, value=10, step=1)

if st.sidebar.button("Download Images"):
    if image_query:
        zip_filename = download_images(image_query, image_limit)
        with open(zip_filename, 'rb') as f:
            st.sidebar.download_button(
                label="Download Image Dataset",
                data=f,
                file_name=zip_filename,
                mime='application/zip'
            )
        st.sidebar.success("Images downloaded and zipped successfully.")
    else:
        st.sidebar.error("Please enter title again.")
